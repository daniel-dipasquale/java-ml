package com.experimental.cliff.common;
...
public class ConcurrentAutoTable implements Serializable {
    private static final AtomicReferenceFieldUpdater<ConcurrentAutoTable, CAT> CAT_UPDATER = AtomicReferenceFieldUpdater.newUpdater(ConcurrentAutoTable.class, CAT.class, "cat");
    // The underlying array of concurrently updated long counters
    private volatile CAT cat = new CAT(null, 4 /*Start Small, Think Big!*/, 0L);
    ...
    private long add_if_mask(final long x, final long mask) {
    ...
    private boolean CAS_cat(final CAT oldcat, final CAT newcat) {
        return CAT_UPDATER.compareAndSet(this, oldcat, newcat);
    }
    ...
    private static class CAT implements Serializable {
        ...
        private static final int L_BASE = UNSAFE.arrayBaseOffset(long[].class);
        private static final int L_SCALE = UNSAFE.arrayIndexScale(long[].class);
        ...
        public long add_if_mask(long x, long mask, int hash, ConcurrentAutoTable master) {
            ...
            while (!_resizerUpdater.compareAndSet(this, r, r + newbytes))
                r = _resizers;
            r += newbytes;
            if (master.cat != this) return old; // Already doubled, don't bother
