...
public final class AverageValuedTileRewardHeuristic implements RewardHeuristic<GameAction, GameState> {
    ...
    private static String createAssertionMessage(final GameState state, final float average, final float maximum) {
    ...
    public float estimate(final GameState state) {
        ...
        for (int tileId = 0; tileId < Game.BOARD_SQUARE_LENGTH; tileId++) {
            int value = state.getExponentialValue(tileId);
        ...
        assert Float.compare(average, maximum) <= 0 : createAssertionMessage(state, average, maximum);
