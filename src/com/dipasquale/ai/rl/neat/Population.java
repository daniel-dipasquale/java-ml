...
final class Population<T extends Comparable<T>> {
    ...
    Population(final Context<T> context) {
        ...
        this.organismsWithoutSpecies = createOrganisms(context, this, genesisGenome);
//        this.organismsToSpecies = new IdentityHashMap<>();
        this.allSpecies = new ArrayList<>();
        this.generation = 1;
    }

    private static <T extends Comparable<T>> Set<Organism<T>> createOrganisms(final Context<T> context, final Population<T> population, final GenomeDefault<T> genesisGenome) {
