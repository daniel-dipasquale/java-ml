...
final class Constants {
    ...
    private static RandomSupport getRandomSupport(final RandomType type, final boolean contended) {
        if (!contended) {
            return switch (type) {
    ...
    static RandomSupport createRandomSupport(final RandomType type, final boolean contended) {
        return new ProxyRandomSupport(type, contended);
    }
    ...
    private static final class ProxyRandomSupport implements RandomSupport, Serializable {
        ...
        private final boolean contended;

        @Override
        public float next() {
            return getRandomSupport(type, contended).next();
        }
    }
}
