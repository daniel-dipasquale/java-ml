...
final class SequentialMap<TId extends Comparable<TId>, TItem> implements Iterable<TItem> {
    ...
    public Iterable<JoinEntry> fullJoin(final SequentialMap<TId, TItem> other) {
    ...
    @RequiredArgsConstructor(access = AccessLevel.PACKAGE)
    private final class JoinIterator implements Iterator<JoinEntry> {
        ...
        private JoinEntry nextUsingBothIterators() {
            iterator1.moveNext = true;
            iterator2.moveNext = true;

            return new JoinEntry(iterator1.entry.getValue(), iterator2.entry.getValue());
        }

        private JoinEntry nextUsingIterator1() {
            iterator1.moveNext = true;

            return new JoinEntry(iterator1.entry.getValue(), null);
        }

        private JoinEntry nextUsingIterator2() {
            iterator2.moveNext = true;

            return new JoinEntry(null, iterator2.entry.getValue());
        }

        @Override
        public JoinEntry next() {
    ...
    public final class JoinEntry {
        private final TItem item1;
        private final TItem item2;
    }
}
